...
Function Rg_Hstack(ParamArray args() As Variant) As Variant
    ...
    For i = LBound(args) To UBound(args)
        If rowCount = 0 Then
            rowCount = UBound(args(i), 1) - LBound(args(i), 1) + 1
        End If
        colCount = colCount + UBound(args(i), 2) - LBound(args(i), 2) + 1
    Next i
    ReDim result(1 To rowCount, 1 To colCount)
    colCount = 0
    For i = LBound(args) To UBound(args)
        For j = LBound(args(i), 1) To UBound(args(i), 1)
            For k = LBound(args(i), 2) To UBound(args(i), 2)
                colCount = colCount + 1
                result(j, colCount) = args(i)(j, k)
            Next k
        Next j
    Next i
    Rg_Hstack = result
End Function
Function Rg_Hstack2(ParamArray arg()) As Variant
    Dim result As Variant
