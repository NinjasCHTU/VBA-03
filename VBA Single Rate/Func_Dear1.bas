...
'Still got the error because right_part is 1d but A_HStackH1 works only for 2d
        right_part = A_1dTo2d(right_part)
        out_arr = A_HStackH1(left_part, right_part)
        D_Combi2 = out_arr
        Exit Function
...
Function Dc_toDict(rngArr_key As Variant, rngArr_val As Variant) As Object
    
      Dim dict As Object
      Set dict = CreateObject("Scripting.Dictionary")
      
      If Not A_IsArray(rngArr_key) Then
        rngArr_key = A_toArray1d(rngArr_key)
      End If
      If Not A_IsArray(rngArr_val) Then
        rngArr_val = A_toArray1d(rngArr_val)
      End If
      
      If UBound(rngArr_key) = UBound(rngArr_val) Then
        For i = LBound(rngArr_key) To UBound(rngArr_key)
          dict.Add rngArr_key(i), rngArr_val(i)
        Next i
      Else
        MsgBox "Error: Arrays have different dimensions"
        Set Dc_toDict = Nothing
        Exit Function
      End If
